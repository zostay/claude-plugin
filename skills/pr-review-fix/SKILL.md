---
name: pr-review-fix
description: Check out a PR, gather every review on it — human and automated — evaluate each finding for validity, fix the good ones, then reply to and resolve the threads on GitHub. If no current automated review exists, always generate one locally — an external CLI (copilot, codex) if one is installed, otherwise Claude's own code-review skill — and post it before fixing.
---
...
changes and the PR's own description, which it discovers itself. A third-party
CLI is worth using when one is installed — a different model notices different
things — but every path below produces a real review. Take the first one that is
...
3. **else** → **Claude's `code-review` skill** (invoke the skill with the PR
   number as its target). This is a complete review, not a degraded one, and its
   findings get exactly the same treatment as any other. If that skill is
   unavailable, dispatch a `general-purpose` Claude Code subagent in a fresh
   context that performs the review and returns the review text.
...
- **The automated review source** — an existing agent review, or one generated by
  ...
  State it as a fact in a clause; a review from Claude's own `code-review` needs
  no explanation, apology, or disclaimer
